...
export default function AccountSettings({ onClose }: Props) {
  ...
  const { user, enableTwoFA, twoFA } = useAuth()
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="relative p-2 rounded-md bg-gray-50 dark:bg-gray-900/30">
            <User size={20} />
            {twoFA ? <ShieldCheck size={12} className="absolute bottom-0 right-0 text-green-500 bg-white rounded-full" /> : null}
          </div>
          <div>
      ...
      <div className="mt-6 space-y-4">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-900/20 rounded-lg flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="text-sm font-medium text-gray-700 dark:text-gray-200">{twoFA ? 'Enabled' : 'Disabled'}</div>
            <button
              onClick={() => enableTwoFA()}
              className={`h-6 w-12 rounded-full p-0.5 transition-colors ${twoFA ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-700'}`}
              aria-pressed={twoFA}
            >
              <div className={`h-5 w-5 rounded-full bg-white transform transition-transform ${twoFA ? 'translate-x-6' : 'translate-x-0'}`} />
            </button>
          </div>
        </div>
        ...
        <div className="flex justify-end">
          <button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
