...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const signInClient = {
    ...
    create: async ({ emailAddress, password }: SignInParams): Promise<SignInResult> => {
      try {
        const response = await axios.post(`https://13.48.71.104.nip.io/api/auth/login`, {
          email: emailAddress,
          password: password,
  ...
  const signUpClient = {
    ...
    create: async ({ emailAddress, password, fullname, role }: SignUpParams): Promise<SignUpResult> => {
      setIsSignedup(true);
      try {
        const response = await axios.post(`https://13.48.71.104.nip.io/api/auth/register`, {
          email: emailAddress,
          password: password,
          fullname: fullname,
          role: role
  ...
  const SignOutButton = async () => {
    await axios.get("https://13.48.71.104.nip.io/api/auth/logout", {withCredentials: true});
    setUser(null)
  ...
  const EditUserDetails = async ({fullname, description, location, phone}) => {
    try{
    const res = await axios.post(`https://13.48.71.104.nip.io/api/users`, { email: user?.email, fullname: fullname, description: description, location: location, phone: phone }, { withCredentials: true });
      if (res.data.success) {
...
useEffect(() => {
  async function fetchUserFromToken() {
    try{
      const res = await axios.get('https://13.48.71.104.nip.io/api/auth/get-cookie', { withCredentials: true });
      if(res.data.success) {
...
const enableTwoFA = async () => {
  ...
  try{
    const res = await axios.post('https://13.48.71.104.nip.io/api/users/enable-two-factor', {email: user?.email, inp: !user?.twoFA}, { withCredentials: true });
    if(res.data.success) {
