...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const signInClient = {
    ...
    create: async ({ emailAddress, password }: SignInParams): Promise<SignInResult> => {
      try {
        const response = await axios.post(`http://13.48.71.104:5000/api/auth/login`, {
          email: emailAddress,
          password: password,
  ...
  const signUpClient = {
    ...
    create: async ({ emailAddress, password, fullname, role }: SignUpParams): Promise<SignUpResult> => {
      setIsSignedup(true);
      try {
        const response = await axios.post(`http://13.48.71.104:5000/api/auth/register`, {
          email: emailAddress,
          password: password,
          fullname: fullname,
          role: role
  ...
  const SignOutButton = async () => {
    await axios.get("http://13.48.71.104:5000/api/auth/logout", {withCredentials: true});
    setUser(null)
  ...
  const EditUserDetails = async ({fullname, description, location, phone}) => {
    try{
    const res = await axios.post(`http://13.48.71.104:5000/api/users`, { email: user?.email, fullname: fullname, description: description, location: location, phone: phone }, { withCredentials: true });
      if (res.data.success) {
...
useEffect(() => {
  async function fetchUserFromToken() {
    try{
      const res = await axios.get('http://13.48.71.104:5000/api/auth/get-cookie', { withCredentials: true });
      if(res.data.success) {
...
const enableTwoFA = async () => {
  ...
  try{
    const res = await axios.post('http://13.48.71.104:5000/api/users/enable-two-factor', {email: user?.email, inp: !user?.twoFA}, { withCredentials: true });
    if(res.data.success) {
