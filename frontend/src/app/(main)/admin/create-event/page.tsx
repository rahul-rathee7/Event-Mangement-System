...
import { z } from 'zod'
import { Loader2, Calendar, Clock, MapPin, DollarSign, Tag, Users, Info, ChevronRight, ChevronLeft, CheckCircle2, ImageIcon, X, Plus } from 'lucide-react'
...
const CreateEventPage = () => {
  ...
  const isStepValid = useCallback((step) => {
    switch(step) {
      case 0:
        return !errors.name && !errors.shortDescription && !errors.description;
      case 1:
        return !errors.startDate && !errors.location && !errors.category;
      case 2:
        return !errors.ticketPrice && !errors.capacity;
      default:
        return true;
    }
  }, [errors]);
  ...
  const goToStep = useCallback((step) => {
    if (step <= currentStep || (step > 0 && isStepValid(step - 1))) {
      setCurrentStep(step)
      window.scrollTo({ top: 0, behavior: 'smooth' })
    }
              ...
              >
                ...
                <FormField label="Tags">
                  <Controller
                    control={control}
                    name="tags"
                    fallback={<div className="py-2 pl-3 h-10 bg-gray-100 dark:bg-gray-700 rounded-md animate-pulse"></div>}
                    render={({ field }) => (
                      <TagInput
                        value={field.value}
                        onChange={field.onChange}
              ...
              >
                ...
                <FormField label="Is this a free event?">
                  <div className="flex items-center h-5">
                    <input
                      ...
                      {...register('isFree')}
                      checked={isFree}
                      onChange={() => setValue('isFree', !isFree, { shouldValidate: true })}
                ...
                <FormField label="Ticket Options">
                  <Suspense fallback={<div className="h-32 bg-gray-100 dark:bg-gray-700 rounded-md animate-pulse"></div>}>
                    <TicketOptionsEditor 
                      isFree={isFree}
                      onChange={(options) => console.log('Ticket options updated:', options)}
                    />
                  </Suspense>
                </FormField>
