...
const Page = () => {
  ...
  const handleSendCode = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://13.48.71.104:5000/api/sendmail/forget-password', { email });
      if (res.data?.success) {
        setStep('enterCode');
        setSentAt(Date.now());
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://13.48.71.104:5000/api/sendmail/verify-otp', { email, otp: code });
      if (res.data?.success) {
        setStep('verifyCode');
  ...
  const handleReset = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://13.48.71.104:5000/api/sendmail/reset-password', { email, password: newPassword });
      if (res.data?.success) {
